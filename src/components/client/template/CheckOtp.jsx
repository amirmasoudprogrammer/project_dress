...
function CheckOtp({number}) {
    ...
    return (
        ...
        <div style={{
                backgroundImage: "url('/image/Rectangle 106.svg')",
                width: "100%",
                height: "150px"
            }} className=" flex items-center justify-center rounded">
                <Image src="/image/Group 946.png" className="object-cover" width={120} height={120} alt="image"/>
            </div>
            <div className="mt-16 flex flex-col items-center justify-center">
                <div><CountdownTimer time={time} setTime={setTime}/></div>
                <span className="text-black mt-10">کد تایید ارسال شده به شماره {number} را وارد.</span>
            </div>
            <div>
                <OtpInput number={number}/>
            </div>
            <div className="flex items-center justify-center mb-20" onClick={() => router.back()}>
                <Link href="/Login_Registration" className="text-blue-500">ویرایش شماره</Link>
            </div>
        </div>
    );
}
