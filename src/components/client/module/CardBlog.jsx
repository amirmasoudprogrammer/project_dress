...
function CardBlog({data}) {
    const {id,images, title, content, updated_at, created_at} = data
    console.log(data)
    return (
        <div className="  flex w-[350px] m-auto md:w-full flex-col md:flex-row items-center p-5 shadow-2xl rounded border border-[#CACACA] md:h-[208px] mt-10 ">
            <div>
                <Image className="rounded" src={images[0]?.image_path} width={272} height={216} alt="image"/>
            </div>
            <div className=" flex flex-col justify-evenly w-full h-full ">
                <div className="flex  items-start justify-start md:items-start md:justify-between mr-5 mt-2  ">
                    <div className="mt-5 md:mt-0">
                        ...
                        <p className="text-[#626262] text-[12px] md:text-sm mt-2">{content}</p>
                    </div>
                ...
                <div className="flex mr-5 relative top-6 mb-10 md:mb-0 text-black justify-between items-center md:w-full ">
                    <div className="flex items-center ">
                        <span className=" text-[10px] md:text-sm md:ml-5 w-[100px] md:w-auto">{new Date(updated_at).toLocaleDateString('fa-IR')}</span>
                        |
                        <p className="text-[10px] md:text-sm mr-2 md:mr-5 text-[#626262]">{new Date(created_at).toLocaleDateString('fa-IR')}</p>
                    </div>
